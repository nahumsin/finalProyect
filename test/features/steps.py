...
def cuando_se_dan_de_alta(step):
    world.result = world.altasBajas.alta(
        world.nombre, world.apellido, world.edad, world.email, world.direccion)
...
def entonces_se_muestra_un_mensaje_de_group1(step, resEsperado):
    assert world.result == resEsperado, 'El resultado esperado %s y el esperado es %s' % (
        world.result, resEsperado)
